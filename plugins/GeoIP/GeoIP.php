...
//		$this->archiveDayAggregateGoals($archiveProcessing);
	...
	protected function archiveDayAggregateVisits(Piwik_ArchiveProcessing $archiveProcessing)
	{
		...
		$labelSQL = 'location_geoip_continent';
	...
	public function updateExistingVisitsWithGeoIpData()
	{
		...
		$stmt = $db->query("SELECT count(*) as cnt FROM ".Piwik_Common::prefixTable('log_visit'), array() );
		$row = $stmt->fetch();
		...
		while( $start < $count )
		{
			$stmt = $db->query("SELECT idvisit, location_ip 
